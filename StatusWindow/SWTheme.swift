...
enum SWMetrics {
    ...
    /// Cap for the reading column. An iPad is ~1024 pt wide and a card stretched
    /// that far is an unreadable line length, so the column is capped and centred
    /// rather than stretched, with the background still running edge to edge.
    /// Every iPhone is narrower than this, so the cap never engages on a phone.
    static let contentMaxWidth: CGFloat = 720
    /// The same idea for the tab bar: five buttons spread over 1024 pt look lost.
    static let tabBarMaxWidth: CGFloat = 640
    /// Narrower column for the single-purpose centred screens (the lock screen).
    static let narrowMaxWidth: CGFloat = 460

    /// The width the app is actually laid out in.
    ///
    /// On iPhone this is exactly `screenWidth`. On iPad it is the window, which
    /// equals the screen when full screen and is smaller in a split view —
    /// `UIScreen` keeps reporting the whole display there, and that is precisely
    /// what makes a screen-derived fixed width overflow its container.
    static var layoutWidth: CGFloat {
        guard UIDevice.current.userInterfaceIdiom == .pad else { return screenWidth }
        let scenes = UIApplication.shared.connectedScenes.compactMap { $0 as? UIWindowScene }
        guard let window = scenes.compactMap({ $0.keyWindow }).first,
              window.bounds.width > 0 else { return screenWidth }
        return min(screenWidth, window.bounds.width)
    }

    /// The width page content is really laid out in, for a view that knows its
    /// horizontal size class. Mirrors `swColumnCap` exactly — capped only where
    /// the cap is actually applied.
    ///
    /// Anything given a *fixed* frame that has to line up with the content column
    /// (a Canvas chart, for one) must be derived from this and never from
    /// `screenWidth`, or it overflows its card on iPad.
    static func contentWidth(for sizeClass: UserInterfaceSizeClass?) -> CGFloat {
        SWLayout.isPadLayout(sizeClass) ? min(layoutWidth, contentMaxWidth) : layoutWidth
    }
}

// MARK: - Adaptive layout
//
// The app is one column of cards. On an iPhone that column IS the screen; on an
// iPad it is capped and centred, because a card stretched across 1024 pt is a
// wall of text. The background underneath stays full bleed either way.

enum SWLayout {
    /// Both halves of this test matter. A size-class-only check would drag the
    /// iPad column onto an iPhone Plus/Max held in landscape, which also reports
    /// a regular width class.
    static func isPadLayout(_ sizeClass: UserInterfaceSizeClass?) -> Bool {
        sizeClass == .regular && UIDevice.current.userInterfaceIdiom == .pad
    }
}

/// Caps a column at a readable width and centres it — on iPad only. On compact,
/// and therefore on every iPhone, the view is handed back exactly as it came in.
struct SWColumnCap: ViewModifier {
    @Environment(\.horizontalSizeClass) private var hSize
    let maxWidth: CGFloat

    @ViewBuilder
    func body(content: Content) -> some View {
        if SWLayout.isPadLayout(hSize) {
            content
                .frame(maxWidth: maxWidth)
                .frame(maxWidth: .infinity)   // centres the column inside the full width
        } else {
            content
        }
    }
...
extension View {
    /// Caps this view at a readable column width on iPad and centres it inside
    /// the full width. A no-op on every iPhone, in both orientations.
    func swColumnCap(_ maxWidth: CGFloat = SWMetrics.contentMaxWidth) -> some View {
        modifier(SWColumnCap(maxWidth: maxWidth))
    }
}
